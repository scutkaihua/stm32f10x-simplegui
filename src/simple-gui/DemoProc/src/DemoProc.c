...
HMI_ENGINE_RESULT InitializeHMIEngineObj(void)
{
	...
	g_stDeviceInterface.stSize.iWidth = 320;
	g_stDeviceInterface.stSize.iHeight = 240;
...
#else
{
	extern void SGUI_SDK_SetPixel(int iX, int iY, int iPixelValue);
	extern int SGUI_SDK_GetPixel(int iX, int iY);
	extern void SGUI_SDK_RefreshDisplay(void);
	extern void SGUI_SDK_ClearDisplay(void);
	extern void SGUI_SDK_SyncKeyEventData(unsigned int uiKeyCode);
	extern unsigned int SGUI_SDK_GetKeyEventData(void);
...
bool SysTickTimerTriggered(void)
{
	...
	{
		extern bool CheckEventFlag(int idx);
		return CheckEventFlag(1);
	}
#endif
}
...
void SysTickTimerEnable(bool bEnable)
{
	...
	{
		extern void SGUI_SDK_ConfigHearBeatTimer(int ms);
		SGUI_SDK_ConfigHearBeatTimer(10);
	}
#endif
}
...
void RTCTimerEnable(bool bEnable)
{
	...
	{
	  extern void SGUI_SDK_EnableRTCInterrupt(bool bEnable);
	  (void)SGUI_SDK_EnableRTCInterrupt(bEnable);
	}
#endif
}
