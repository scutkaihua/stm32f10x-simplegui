...
// </h>
static GPIO_TypeDef* const GPIO_TABLE[] = {GPIOA, GPIOB, GPIOC, GPIOD, GPIOE, GPIOF, GPIOG};
static const U32 RCC_GPIO_TABLE[] = {RCC_APB2Periph_GPIOA,RCC_APB2Periph_GPIOB,RCC_APB2Periph_GPIOC,RCC_APB2Periph_GPIOD,RCC_APB2Periph_GPIOE,RCC_APB2Periph_GPIOF,RCC_APB2Periph_GPIOG};
...
*/
int cpu_gpio_cfg(U8 xPort,U8 xPin,U8 xMode,U8 xSpeed,U8 sta)
{
    GPIO_InitTypeDef xGpioInit;
    GPIO_TypeDef* GPIOx;
    if(xPort>7)return FALSE;
    ...
    RCC_APB2PeriphClockCmd(RCC_GPIO_TABLE[xPort], ENABLE);
    GPIO_Init(GPIOx, &xGpioInit);     
	/*当作为输出时，初始化输出电平*/
    if ( xMode == GPIO_Mode_Out_OD || xMode == GPIO_Mode_Out_PP )
    {
